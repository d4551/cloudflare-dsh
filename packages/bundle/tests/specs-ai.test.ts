...
describe('Workers AI specs', () => {
  ...
  it('adds only the search filter when the task is omitted', () => {
    expect(aiModelsSearchSpec('llama', undefined, 10).query).toStrictEqual({ per_page: 10, search: 'llama' })
  })
...
describe('AI Gateway specs', () => {
  ...
  it('fetches one gateway', () => {
    expect(gatewayGetSpec('gw1')).toStrictEqual({ method: 'GET', path: '/ai-gateway/gateways/gw1' })
  })
  ...
  it('queries logs with a page size', () => {
    expect(gatewayLogsSpec('gw1', 50, {})).toStrictEqual({
      method: 'GET',
  ...
  it('merges defined filters into the log query', () => {
    expect(gatewayLogsSpec('gw1', 10, { 'metadata.sessionId': 's1' }).query).toStrictEqual({
      per_page: 10,
      'metadata.sessionId': 's1',
    })
  })

  it('drops undefined filters rather than sending empty parameters', () => {
    expect(gatewayLogsSpec('gw1', 10, { a: undefined, b: 'x' }).query).toStrictEqual({ per_page: 10, b: 'x' })
  })
  ...
  it.each(['credit-balance', 'usage-history', 'invoice-preview'] as const)('reads the %s billing view', (view) => {
    expect(gatewayBillingSpec(view)).toStrictEqual({ method: 'GET', path: `/ai-gateway/billing/${view}` })
  })
})
...
describe('AI Search specs', () => {
  ...
  it('triggers a sync job', () => {
    expect(aiSearchSyncSpec('i1')).toStrictEqual({ method: 'POST', path: '/ai-search/instances/i1/jobs' })
  })
...
describe('Vectorize specs', () => {
  it('lists indexes under the v2 path', () => {
    expect(vectorizeIndexListSpec()).toStrictEqual({ method: 'GET', path: '/vectorize/v2/indexes' })
  })
  ...
  it('queries an index', () => {
    expect(vectorizeQuerySpec('idx', [0.1, 0.2], 5, false, true)).toStrictEqual({
      ...
      body: { vector: [0.1, 0.2], topK: 5, returnValues: false, returnMetadata: 'all' },
    })
  })
